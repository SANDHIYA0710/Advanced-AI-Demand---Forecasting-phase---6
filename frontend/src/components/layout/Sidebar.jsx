...
import {
  ...
  MdBusiness, MdGavel, MdAccountTree, MdLeaderboard, MdShield,
} from "react-icons/md";
...
const navGroups = [
  ...
  { label:"Intelligence", items:[
    ...
    { to:"/accuracy",    label:"Accuracy",     icon:MdGrade },
  ...
  { label:"Enterprise", items:[
    { to:"/organizations", label:"Organizations", icon:MdBusiness },
    { to:"/approvals",     label:"Approvals",     icon:MdGavel },
    { to:"/workflows",     label:"Workflows",     icon:MdAccountTree },
    { to:"/kpis",          label:"KPIs",          icon:MdLeaderboard },
    { to:"/governance",    label:"Governance",    icon:MdShield },
  ]},
...
export default function Sidebar() {
    ...
    <>
      <div className="px-4 py-4 border-b border-primary-100 dark:border-primary-900 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="font-display font-bold text-sm" style={{ color:"var(--text)" }}>ForecastIQ</span>
            <p className="text-[10px] text-primary-500">Enterprise v6.0</p>
          </div>
        </div>
        <button onClick={closeMobile} className="md:hidden text-primary-400"><MdClose className="text-xl" /></button>
      </div>
            ...
            <div className="space-y-0.5">
              {items.map(({ to, label:l, icon:Icon }) => (
                <NavLink key={to} to={to} onClick={closeMobile}
                  ...
                  <Icon className="text-lg flex-shrink-0" />{l}
                </NavLink>
              ))}
            </div>
          </div>
      ...
      <div className="px-3 pb-4 border-t border-primary-100 dark:border-primary-900 pt-3">
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-primary-50 dark:bg-primary-900/50 mb-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-primary-500 capitalize">{user?.is_admin ? "👑 Super Admin" : user?.role || "analyst"}</p>
          </div>
        </div>
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(true)} className="md:hidden fixed top-4 left-4 z-50 w-9 h-9 bg-white dark:bg-primary-950 border border-primary-200 dark:border-primary-800 rounded-xl flex items-center justify-center shadow-glass">
      ...
      <aside className={`md:hidden fixed top-0 left-0 h-full w-60 backdrop-blur-md border-r flex flex-col z-50 transition-transform duration-300 bg-white/95 dark:bg-primary-950/95 border-primary-100 dark:border-primary-900 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <Content />
      </aside>
      <aside className="hidden md:flex w-56 min-h-screen backdrop-blur-md border-r flex-col bg-white/80 dark:bg-primary-950/80 border-primary-100 dark:border-primary-900 transition-colors duration-300">
        <Content />
      </aside>
    </>
  );
